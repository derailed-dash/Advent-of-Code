...
- If I leave the map facing up from face `0`, then I'm leaving at edge `a`. So I'll reappear on the map in face `1`, but I'll be travelling _downwards_.  Why? Because the `a` arrow for face `1` is at 180 degrees, relative to the `a` arrow for face `0`. And consequently, if I was travelling up when I left face `0`, then I must be now facing down when I enter face `1`.
