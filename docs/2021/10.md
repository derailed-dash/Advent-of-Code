...
Unsurprisingly, we're now asked to work with the _incomplete_ lines. Specifically, we're told we need to complete the incomplete lines, by adding the necessary sequence of closing brackets. 
...
The `get_completion_for_line()` function is the same as Solution 1.
...
21:04:20.968:INFO:__main__:     Execution time: 0.0039 seconds
```
