...
<grn>A    0    0    1    6    C    8    8    0    1    6    2    0    1    7    C    3    6    8    6    B    1    8    A    3    D    4    7    8    0    (HEX)</grn>
...
- Our `hex_to_bin()` method converts all the hex digits to binary digits, by:
  - First converting from hex to decimal, using `int(value, base)`, where `base=16`.
  - Then converting the decimal number to binary, by applying a `format spec`, where:
    - `{0}` is the first parameter to the `format()` method, i.e. the decimal value;
    - And `:0{width}b` tells `format()` to convert to `binary` type, and then pad with `0` in order to achieve the desired `str` length.rt to binary format.
