...
This code defines a plot area, then uses NetworkX's built in `draw_spring()` method to render the `nx.Graph()` in a `spring layout`, on the plot area. It then saves the rendering as a file, creating the path if it doesn't exist.
...
I've used the RENDER constant so that I can easily turn rendering on and off.
