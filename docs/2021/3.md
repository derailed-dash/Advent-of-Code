...
The only line worth mentioning is the list comprehension that actually does the filtering at the end.  This comprehension checks if the specified position has the bit we want to keep, and only returns the numbers that match.  Finally, when we're only left with one number, we exit the loop and return it.
